...
- 支援上傳或建立 `my_applications.csv`，一次估算多筆候補資料。
- 支援下載 `my_applications.csv` 範本與估算結果 CSV。
- 顯示官方候補資料與名冊概況。
- 透過 GitHub Actions 定期更新公開資料。

## 我的候補估算

公開版新增「我的候補估算」功能。使用者可以下載範本、上傳自己的 `my_applications.csv`，或直接在網頁表格新增多筆候補資料。

`my_applications.csv` 必要欄位如下：

```csv
社會住宅,遞補類型,房型,戶別,我的候補序號
南屯建功1號,隨到隨辦,二房型,一般戶,15
東區恊園,新案場招租,二房型,一般戶,80
```
...
本功能只在當次瀏覽器 session 使用上傳或輸入的資料進行估算，不會把使用者的 `my_applications.csv` 寫入 GitHub，也不會保存到伺服器。若重新整理或關閉頁面，請重新上傳或重新輸入。

## 隱私說明

本工具不需要登入，也不會保存使用者輸入或上傳的候補序號、候補資料或個人資料。

使用者在網頁上輸入或上傳的資料，只會用於當下查詢與估算，不會寫入資料庫，也不會用於通知、追蹤或其他用途。

## 使用方式

開啟 Streamlit 網頁後，可使用兩種方式查詢。
...
1. 下載 `my_applications.csv` 範本。
2. 上傳自己的 `my_applications.csv`。
3. 或直接在表格中新增多筆候補資料。
4. 選擇估算基準。
5. 按下「估算我的候補資料」。
6. 下載估算結果 CSV。
